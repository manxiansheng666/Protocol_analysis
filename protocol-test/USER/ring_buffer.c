...
{
	if(ring_buffer_handle->lenght < keyword_lenght)
		return RING_BUFFER_ERROR ;//已有数据比关键词短，直接退出
 ...
 * \return 返回执行结果
*/
void Ring_Buffer_Clear(ring_buffer *ring_buffer_handle)
{
    Ring_Buffer_Delete(ring_buffer_handle,ring_buffer_handle->lenght);
}
